...
void initialize_physics_objects(std::vector<std::string> args) {
    ...
    // large collidable sphere at (0,0) (i never ended up making it a bowl)
    ...
    // skybox
    globals::physics_objects.push_back(std::make_shared<physics_object::object>(physics_object::blueprints::sphere(vector::worldspace(0, 0, 0), 1e8)));
